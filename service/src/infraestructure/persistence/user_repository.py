...
from src.service.controller.user_controller import get_users
        ...
        try:
            ...
            if email_exists.scalar():
                create_user_response = CreateUserResponse(
                    is_success = False,
                    message = str("User already exists with that email"),
                ...
                if not PasswordUtils.is_password_valid(generic_user_request.password):
                    create_user_response = CreateUserResponse(
                        is_success = False,
                        message = str("Invalid password, the password must be at least 8 characters long, one uppercase letter, one number, one lowercase letter and one special character"),
                        response_codes_json = ResponseCodesJson.BAD_REQUEST,
                    )
                else:
                    if (len(generic_user_request.dni) != 9
                        ...
                        create_user_response = CreateUserResponse(
                            is_success = False,
                            message = str("Invalid DNI, the email need 8 consecutive numbers and one uppercase letter"),
                            response_codes_json = ResponseCodesJson.BAD_REQUEST,
                        )
                    else:
                        ...
                        if dni_exists.scalar():
                            create_user_response = CreateUserResponse(
                                is_success = False,
                                message = str("User already exist with that dni"),
                                response_codes_json = ResponseCodesJson.BAD_REQUEST,
                            )
                        else:
